        ...
        if not os.path.exists(mtlpath):
            ...
            for line in mtl:  # .readlines():
                ...
                if not line or line.startswith(b'#'):
                    ...
                    if line_lower.startswith(b'ka'):
                    ...
                    elif line_lower.startswith(b'd'): # case dissolved
                        context_material.alpha = float(line_split[1])
                        if context_material.alpha < 1.0:
                            context_material.use_transparency = True
                            context_material.transparency_method = 'Z_TRANSPARENCY'
                        else:
                            context_material.use_transparency = False
                    elif line_lower.startswith(b'tf') or line_lower.startswith(b'tr'): # case transparent 
                        context_material.alpha = float(line_split[1])
                        if context_material.alpha < 1.0:
                            context_material.use_transparency == True
                        ...
                        if illum == 0:
                            ...
                            do_ambient = False # make shadeless
                        elif illum == 1:
                            # Color on and Ambient on
                            do_ambient = True # use no spec
                        elif illum == 2:
                            # Highlight on 
                            do_highlight = True
                        elif illum == 3:
                        ...
                        context_material.ambient = 1.0 # usual case - ambient, std spec
                        context_material.specular_intensity = 0.5
                        if do_ambient == False: # no ambient, shadeless, use an avg spec
                            context_material.ambient = 0.0
                            context_material.specular_intensity = 0.5                         
                            context_material.use_shadeless = True
                        if do_ambient == True: # ambient, use no spec
                            context_material.ambient = 1.0
                            context_material.specular_intensity = 0.0                          

                        if do_highlight: # give more spec than std
                            context_material.specular_intensity = 0.8
                                
                        if do_reflection:
                            context_material.raytrace_mirror.use = True
                            context_material.raytrace_mirror.reflect_factor = 0.15 # use a handy value

                        if do_transparency:
                            context_material.use_transparency = True
                            context_material.transparency_method = 'RAYTRACE' if do_raytrace else 'Z_TRANSPARENCY'
                            # use read alpha value

                        if do_glass:
                            pass # use read Ni(IOR) value
